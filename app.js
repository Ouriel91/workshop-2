...
function cleanUpUiAfterAnswer(name, avengers, answer) {
    ...
    if(!answer){
        if(name){ //null check for stone click
...
glove.addEventListener('click', () => {
    ...
    //validations - no stones/avengers selected or more than need avengers selected
    if(objCheck.avengers.length > 2 || objCheck.name == '' || objCheck.avengers.length === 0){
...
const stopConfetti = (displayStones) => {
    setTimeout(function() {
        confetti.stop()
